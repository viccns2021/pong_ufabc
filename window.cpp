...
void Window::onPaintUI() {
...

  {
    ...
    auto const position{ImVec2((m_viewportSize.x - size.x) / 2.0f, 0.1f)};
    ...
    std::minstd_rand rng(m_gameData.ct);
    std::uniform_real_distribution<float> colorDist(0.0f, 1.0f);
    ImVec4 textColor = ImVec4(colorDist(rng), colorDist(rng), colorDist(rng), 1.0f);

    // Set text color
    ImGui::TextColored(textColor, "bounces: %s", std::to_string(m_gameData.ct).c_str());
